...
int main() {
  ...
  while (sc1 != 21 && sc2 != 21) {
    ...
    //если коснулся ракетки
              ...
              (x_b == x_r1 || x_b == x_r1 + 1 || x_b == x_r1 - 1)) ||
             (y_b == 70 &&
              (x_b == x_r2 || x_b == x_r2 + 1 || x_b == x_r2 - 1))) {
      y_move *= -1;
    }
...
void draw(int sc1, int sc2, int x_r1, int x_r2, int x_b, int y_b) {
  for (int i = 0; i < 25; i++) {
    for (int j = 0; j < 80; j++) {
      if (i == 0 || i == 24) {
        printf("-");
      } else if (j == 0 || j == 79 || (j == 40 && !(i == x_b && j == y_b))) {
        printf("|");
      } else if (((i == x_r1 || i == x_r1 + 1 || i == x_r1 - 1) && j == 9) ||
                 ((i == x_r2 || i == x_r2 + 1 || i == x_r2 - 1) && j == 71)) {
        printf("#");
      } else if (i == x_b && j == y_b) {
        printf("O");
      } else {
